    ...
    def capture_sequence(self):
        try:
            ...
            for idx, (i, led) in enumerate(leds, start=1):
                ...
                raw_name = os.path.join(IMAGE_DIR, f"raw/{timestamp}_LED{i}_raw_.png")
                norm_name = os.path.join(IMAGE_DIR, f"norm/{timestamp}_LED{i}_norm.png")
                norm_ratio_name= os.path.join(IMAGE_DIR, f"{timestamp}_LED{i}_ratio.tiff")

                cv2.imwrite(raw_name, img)         
                cv2.imwrite(norm_name, img_norm)
                cv2.imwrite(norm_ratio_name, norm_ratio.astype(np.float32))
