...
class PeanutApp(tk.Tk):
    def __init__(self):
        ...
        self.latest_heatmap_photo = None
    ...
    def _create_widgets(self):
        ...
        self.start_btn.grid(row=1, column=0, pady=(16, 12), sticky="ew")
        ...
        self.progress_bar.grid(row=2, column=0, pady=(0, 16), sticky="ew")

        data_card = ttk.Labelframe(left_cap, text="Data Collection", style="Card.TLabelframe", padding=8)
        data_card.grid(row=3, column=0, sticky="ew", pady=(0, 10))
        ...
        ttk.Label(result_card, textvariable=self.total_var, style="Result.TLabel").grid(row=0, column=0, sticky="w", pady=4)
        ttk.Label(result_card, textvariable=self.black_var, style="Result.TLabel").grid(row=1, column=0, sticky="w", pady=4)
        ttk.Label(result_card, textvariable=self.brown_var, style="Result.TLabel").grid(row=2, column=0, sticky="w", pady=4)
        ...
        ttk.Label(left_analysis, text="Saved Maturity Heatmaps", style="Subtitle.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 6))
        ...
        self.refresh_analysis_btn = ttk.Button(left_analysis, text="Refresh Heatmaps", style="Secondary.TButton", command=self.refresh_analysis_list)
        self.refresh_analysis_btn.grid(row=2, column=0, pady=(8, 0), sticky="ew")
    ...
    def safe_update_results(self, result: dict):
        def _update():
            ...
            self.show_latest_heatmap_preview(self.latest_heatmap_path)
            self.refresh_analysis_list()

            if self.latest_heatmap_path and os.path.exists(self.latest_heatmap_path):
                self.after(2000, self.clear_heatmap_preview)
    ...
    def show_latest_heatmap_preview(self, filepath: str | None):
        if not filepath or not os.path.exists(filepath):
            self.heatmap_preview_label.config(text="No heatmap available")
            self.latest_heatmap_photo = None
            return
        ...
        try:
            ...
            max_w, max_h = 480, 320
            img.thumbnail((max_w, max_h))
            self.latest_heatmap_photo = ImageTk.PhotoImage(img)
            self.heatmap_preview_label.config(image=self.latest_heatmap_photo, text="")
        except Exception as e:
            self.heatmap_preview_label.config(text=f"Error loading heatmap:\n{e}")
            self.latest_heatmap_photo = None

    def clear_heatmap_preview(self):
        self.heatmap_preview_label.config(text="Heatmap preview finished")
        self.latest_heatmap_photo = None
    ...
    def refresh_analysis_list(self):
        ...
        files = [
            f for f in os.listdir(ANALYSIS_OUTPUT_DIR)
            if f.lower().endswith("_maturity_heatmap.png")
        ]
        files.sort(key=lambda f: os.path.getmtime(os.path.join(ANALYSIS_OUTPUT_DIR, f)), reverse=True)

        for f in files:
            self.analysis_listbox.insert(tk.END, f)

        if files:
            self.analysis_listbox.select_set(0)
            self.analysis_listbox.event_generate("<<ListboxSelect>>")
    ...
    def on_analysis_select(self, event):
        ...
        filename = self.analysis_listbox.get(selection[0])
