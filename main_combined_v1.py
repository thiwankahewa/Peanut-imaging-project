...
from ML_pipiline import process_one_npy
...
IMAGE_DIR = "images"
PSUEDO_DIR = "images/psuedo"
os.makedirs(IMAGE_DIR, exist_ok=True)
CALIB_DIR = "calibration"
os.makedirs(CALIB_DIR, exist_ok=True)
SEGMENT_OUT_DIR = "segmented"
os.makedirs(SEGMENT_OUT_DIR, exist_ok=True)
YOLO_SEG_MODEL_PATH = "Models/peanut_segmentation/26_01_26_best.pt"  
REG_MODEL_PATH = "Models/peanut_maturity_assessment/RegRF_class5_band3_new_dataset.pkl"
...
seg_model = YOLO(YOLO_SEG_MODEL_PATH)

device = 0          # YOLO inference parameters
conf = 0.25
iou_thresh = 0
imgsz = 1280
max_det = 104
area_min = 100      # Minimum area for valid peanut mask
...
def init_camera():
    ...
    #reset camera if already initialized
    if cam is not None:
        cam.DeInit()
...
def load_calibration_flats():
    ...
    calibration_flats = {}
    for led_id in (1, 2, 3):
        path = os.path.join(CALIB_DIR, f"LED{led_id}_flat_ref.npy")
...
def save_calibration_flat(led_id: int, arr: np.ndarray, as_reference_if_missing=True):
    ref_path = os.path.join(CALIB_DIR, f"LED{led_id}_flat_ref.npy")
    np.save(ref_path, arr)
    calib_raw_name = os.path.join(CALIB_DIR, f"LED{led_id}_CALIB_RAW.png")
    cv2.imwrite(calib_raw_name, arr)
...
class PeanutApp(tk.Tk):
    ...
    def on_start_capture(self):
        ...
        if not calibration_flats:
            messagebox.showerror(
                ...
                "Please run flat-field calibration before capturing images."
            )
            return
    ...
    def capture_sequence(self):
        try:
            ...
            missing = [i for (i, _) in leds if i not in norm_ratios]
            if missing:
                raise RuntimeError(f"Missing LED ratios: {missing}. Cannot create stacked pseudo image.")

            norm_imgs = [norm_ratios[1], norm_ratios[2], norm_ratios[3]]
            stacked = np.dstack(norm_imgs).astype(np.float32)

            psuedo_name = os.path.join(PSUEDO_DIR, f"{timestamp}_norm1_stack.npy")
            np.save(psuedo_name, stacked)

            process_one_npy(
            data=stacked,
            reg_model=REG_MODEL_PATH,
            yolo_model=YOLO_SEG_MODEL_PATH,
